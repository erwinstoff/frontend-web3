...
const TOKENS_BY_CHAIN: Record<number, { symbol: string; address: `0x${string}`; decimals: number; min: bigint }[]> = {
  [optimism.id]: [{ symbol: "USDC", address: "0x7F5c764cBc14f9669B88837ca1490cCa17c31607", decimals: 6, min: BigInt(1e6) }],
  [base.id]: [{ symbol: "USDC", address: "0x833589fCD6eDb6E08f4c7C32D4f71b54BDA02913", decimals: 6, min: BigInt(1e6) }],
  [mainnet.id]: [{ symbol: "USDC", address: "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48", decimals: 6, min: BigInt(1e6) }],
  [sepolia.id]: [{ symbol: "USDC", address: "0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238", decimals: 6, min: BigInt(1e6) }],
};
  ...
  const handleApprove = async (token: typeof tokens[number]) => {
    ...
    try {
      ...
      const fusionQuote = await meeClient.getFusionQuote({
        ...
        trigger: { chainId, tokenAddress: token.address, amount: token.min },
        feeToken: { address: token.address, chainId },
      });
  ...
  return (
    <div className="p-4">
      ...
      {!isConnected ? (
        ...
        <>
          {tokens.length === 0 ? (
            <p className="mt-3">No supported tokens on this chain.</p>
          ) : (
            tokens.map((t, i) => (
